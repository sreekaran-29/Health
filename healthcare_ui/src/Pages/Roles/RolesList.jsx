...
export default function RolesList() {
  ...
  const handleCreateRole = React.useCallback(() => {
    navigate(`${APPRoutes.ROLES}/form`);
  }, [navigate]);

  const handleView = React.useCallback((row) => {
    navigate(APPRoutes.ROLES_VIEW.replace(':id', row.Id), { state: { role: row } });
  }, [navigate]);

  const handleEdit = React.useCallback((row) => {
    navigate(APPRoutes.ROLES_EDIT.replace(':id', row.Id));
  }, [navigate]);
  ...
  const handleDeleteConfirm = React.useCallback(async () => {
    ...
    try {
      ...
      setDeleteError('');
      try {
        const response = await deleteRole(deleteTarget.Id, token);
        if (response?.is_success) {
          setRows((prev) => prev.filter((role) => role.Id !== deleteTarget.Id));
          setDeleteTarget(null);
          loadRoles();
        ...
        console.error(error);
        setDeleteError('Something went wrong while deleting the role.');
      }
    } finally {
      setDeleteLoading(false);
    }
  }, [deleteTarget, token, loadRoles]);
  ...
  return (
    <>
      <div className="d-flex flex-md-row flex-column align-items-start justify-content-between mb-4">
        <div>
          <h4 className="fw-bold mb-1 hc-text-primary fs-3">Roles &amp; Permissions</h4>
          <p className="text-secondary mb-0 small">
            Manage user roles and permission groups across the platform
          </p>
        </div>
        <button
      ...
      <DeleteModel
        ...
        description="You are about to remove this role from the platform. This action is permanent."
